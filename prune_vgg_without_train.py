...
if __name__ == "__main__":
    ...
    test_acc = model.evaluate(x_test, y_test, batch_size=256)[1]
    ...
    while 1:
        ...
        for i in range(W.shape[3]):
            ...
            l2_norm = np.linalg.norm(filter_kernel_0) + np.linalg.norm(filter_kernel_1) + np.linalg.norm(
                filter_kernel_1)  # l2-norm of the kernel is computed
        ...
        number_of_channels_to_prune_at_once = int(len(ratio_list) * 0.50)  # remove 50% of the channels at once
        ...
        if (number_of_channels_to_prune_at_once > 1):
            model = train(model, 5)
            ...
            if acc_pruned > 0:
                ...
                test_acc = model.evaluate(x_test, y_test, batch_size=256)
                test_accuracy_list.append(test_acc[1])
        ...
        if conv_index == len(conv_layers) - 1:
            ...
            break
        if (
                number_of_channels_to_prune_at_once == 1):  # 5 percent loss is tolerable, but otherwise move to the next conv layer
            print('not pruning the current layer ! accuracy will drop !')
            print('pruned all filters in layer :: ' + str(conv_index) + '. Moving to the next layer to the left')
